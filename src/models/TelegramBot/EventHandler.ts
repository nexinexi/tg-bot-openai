...
export class EventHandler {
  ...
  onVoiceMessage() {
    this.bot.on(message('voice'), async (ctx) => {
      ...
      try {
        const userVoicesDir = path.resolve(
          __dirname,
          `../../../voices/${ctx.message.from.id}`,
        );

        createDirIfDoesntExists(userVoicesDir);

        const filePath = await this.telegramClient.downloadFile(
          file_id,
          userVoicesDir,
        );
        ...
        await ctx.reply(
          code(
            `${text.endsWith('?') ? 'Ваш вопрос:' : 'Ваше сообщение:'} ${text}`,
          ),
        );
        await ctx.telegram.sendChatAction(ctx.chat.id, 'typing');
